...
void YAMLProcessor::parseGeneral() {
  ...
  std::cout << ss.str();
    try {
        ...
        if (general_node) {
            this->general.personal_info.name = general_node["personal_info"]["name"].as<std::string>();
            this->general.personal_info.location = general_node["personal_info"]["location"].as<std::string>();
            this->general.personal_info.email = general_node["personal_info"]["email"].as<std::string>();
            this->general.personal_info.phone = general_node["personal_info"]["phone"].as<std::string>();
            this->general.personal_info.website = general_node["personal_info"]["website"].as<std::string>();
            this->general.personal_info.linkedin = general_node["personal_info"]["linkedin"].as<std::string>();
            this->general.personal_info.linkedin_handle = general_node["personal_info"]["linkedin_handle"].as<std::string>();
            this->general.personal_info.github = general_node["personal_info"]["github"].as<std::string>();
            this->general.personal_info.github_handle = general_node["personal_info"]["github_handle"].as<std::string>();
            this->general.personal_info.profile_pic = general_node["personal_info"]["profile_pic"].as<std::string>();

            if (auto skills_achievements_node = general_node["skills_achievements"]; skills_achievements_node && skills_achievements_node.IsMap()) {
            ...
            if (auto education_node = general_node["education"]; education_node && education_node.IsSequence()) {
            ...
            if (auto awards_node = general_node["awards"]; awards_node && awards_node.IsSequence()) {
    ...
    try {
        ...
        if (experiences_node && experiences_node.IsSequence()) {
            ...
            for (const YAML::Node& experience_node : experiences_node) {
                ...
                std::vector<std::string> cv_types;
                if (experience_node["cv_type"] && experience_node["cv_type"].IsSequence()) {
                ...
                experience.points =  experience_node["points"].as<std::vector<std::string>>();
#ifdef DEBUG
                std::cout << "- " << experience << "\n\n";
#endif // DEBUG
